// VINTAGE LOOP STATION – VM EDITION
// Optimized for Virtual Machines (UTM Arch Linux, etc.)
// Gracefully handles missing audio devices, JACK server, and HID
// ======================================================

// VM MODE: Reduces audio device requirements
1 => int VM_MODE;
...
HidMsg blueTurnMsg;
0 => int useHid;
0 => int hidAvailable;  // Track if HID is available
...
// Audio setup with VM mode fallback
Gain input => blackhole;
Gain clickGain;

if (VM_MODE)
{
    // In VM mode, create dummy audio path to avoid errors
    <<< "[VM MODE] Audio devices may be limited or unavailable" >>>;
    // Still connect but don't fail if adc/dac unavailable
    input.gain(0.0);  // Mute input in VM mode
    clickGain.gain(0.0);  // Mute click in VM mode
}
else
{
    adc => input;
...
LiSa mainLoop => Gain mainDry => Gain masterGain => Pan2 masterPan;
Gain inputToLoop => mainLoop;

if (VM_MODE)
{
    // VM mode: use oscillator as test signal instead of adc
    SinOsc testTone => inputToLoop;
    0 => testTone.gain;  // Silent by default
    <<< "[VM MODE] Using test tone generator (press 't' to toggle)" >>>;
}
else
{
    adc => inputToLoop;
...
for (0 => int i; i < 10; i++)
{
    inputToLoop => overdubPlayers[i] => overdubDry[i] => overdubPan[i];
    if (!VM_MODE) overdubPan[i] => dac;
...
fun int openBlueTurn()
{
    ...
    {
        <<< "[VM MODE] Could not open BlueTurn (HID not available)" >>>;
        0 => hidAvailable;
        return 0;
    }
    1 => hidAvailable;
...
{
    <<< "[VM MODE ACTIVE]" >>>;
    <<< "  - Reduced audio device requirements" >>>;
    <<< "  - Test tone generator available (press 't')" >>>;
    <<< "  - HID/MIDI controllers disabled" >>>;
}
...
if (VM_MODE)
{
    <<< "[VM MODE] Skipping BlueTurn HID (device access may be limited)" >>>;
}
else
{
...
if (VM_MODE)
{
    <<< "[VM MODE] Skipping MIDI controller (JACK not available)" >>>;
}
else
{
...
while (true)
{
    ...
    // VM mode: 't' toggles test tone instead of tap tempo
    if (k == 't' && VM_MODE)
    {
        if (testTone.gain() > 0)
        {
            0 => testTone.gain;
            "Test tone OFF" => statusMessage;
        }
        else
        {
            0.3 => testTone.gain;
            440.0 => testTone.freq;
            "Test tone ON (440Hz)" => statusMessage;
        }
        showLED(0);
    }
    else if (k == 't' && !freeLoop) spork ~ tapTempo();
