// VINTAGE LOOP STATION - EXACT o* SYMBOLS
...
// showLED writes state to JSON for external display driver
0 => int headerPrinted;
0 => int lastStatePos;  // Track last written position
time lastWriteTime;     // Time-based throttle
50::ms => dur writeInterval;  // Minimum time between writes

// Write state to JSON file for external display driver
fun void writeStateFile(int pos)
{
    // Throttle: only write every 4th position AND respect time interval
    if (pos % 4 != 0 && pos != 0) return;
    if (now - lastWriteTime < writeInterval && pos != 0) return;
    if (pos == lastStatePos && pos != 0) return;
    
    pos => lastStatePos;
    now => lastWriteTime;
    
    // Build track states array
    "" => string trackStates;
    if (mainLoopActive) "1" => trackStates;
    else "0" => trackStates;
    
    for (0 => int i; i < overdubCount && i < 10; i++)
    {
        if (overdubActive[i]) trackStates + ",1" => trackStates;
        else trackStates + ",0" => trackStates;
    }
    
    // Build JSON manually (ChucK has no JSON library)
    "{" => string json;
    json + "\"pos\":" + pos => json;
    json + ",\"ledCount\":" + ledCount => json;
    json + ",\"recording\":" + isRecording => json;
    json + ",\"overdubbing\":" + isOverdubbing => json;
    json + ",\"playing\":" + isPlaying => json;
    json + ",\"loopExists\":" + loopExists => json;
    json + ",\"trackCount\":" + (overdubCount + 1) => json;
    json + ",\"tracks\":[" + trackStates + "]" => json;
    json + ",\"bpm\":" + bpm => json;
    json + ",\"mode\":\"" + (measureMode ? "measure" : "free") + "\"" => json;
    json + ",\"selectedTrack\":" + selectedTrack => json;
    json + "}\n" => json;
    
    // Write to file
    FileIO fout;
    fout.open("/tmp/loopstation_state.json", FileIO.WRITE);
    if (fout.good())
    {
        fout <= json;
        fout.close();
...
// showLED - Updates state file for external display driver
...
fun void showLED(int pos)
{
    ...
    // Write state to file for display driver
    writeStateFile(pos);
}
            ...
            if (k == 10 || k == 13) // ASCII newline or carriage return
            {
                if (input.length() > 0)
                {
                    ...
                    if (newBeats >= 1 && newBeats <= 16)
                    {
                        ...
                        <<< "[OK] Beat length set to", beatsPerLoop, "beats (", ledCount, "LEDs)" >>>;
                        0 => listening;
                    }
                    else
                    {
...
fun int openBlueTurn()
{
    ...
    <<< "[OK] BlueTurn connected:", blueTurn.name() >>>;
    return 1;
}
    ...
    for (0 => int i; i < 8; i++)  // Check first 8 MIDI devices
    {
        if (midiIn.open(i))
        {
            ...
            if (midiIn.name().find("APC") >= 0 || midiIn.name().find("apc") >= 0)
            {
                ...
                midiOut.open(i);
                <<< "[OK] MIDI Controller connected:", midiIn.name() >>>;
                return 1;
            }
            else
            {
...
if (sessionLoaded)
{
    <<< "\n[OK] Session loaded - starting in", (measureMode ? "Measure" : "Free"), "mode" >>>;
    if (loopExists) 
    {
...
while (modeSelected)
{
    ...
    while (kb.more())
    {
        ...
        if (k == '1')
        {
            1 => measureMode;
            0 => modeSelected;
            <<< "\n[OK] MEASURE MODE selected (", beatsPerLoop, "beats per loop)" >>>;
        ...
        else if (k == 'r')
        {
            ...
            0 => measureMode;
            0 => modeSelected;
            <<< "\n[OK] FREE MODE auto-selected (recording started)" >>>;
            <<< "RECORDING: [r] Start/Stop Recording | [o] Overdub" >>>;
            <<< "PLAYBACK: [p] Play/Stop | [u] Undo | [e] Erase All | [0-9] Select/Toggle Track" >>>;
            <<< "EFFECTS: [+/-/=] Speed | [v] Reverse | [{/}] Track Vol | [[/]] Master Vol | [,/.] Pan | [/] Pan Reset" >>>;
            <<< "[s] Save Session | [q] Quit | [x/ESC] Emergency Stop" >>>;
